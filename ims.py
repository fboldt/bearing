...
def files_debug(dirfiles):
  ...
  n = 100 # Fewer files, compared to the original 750

  # Normal
  for i in range(1, n+1):
    key = "Normal_" + str(i).zfill(3)
    files_names[key] = first_test_files[i-1]

  # Inner
  for i in range(1370, n+1370):
    key = "IR_" + str(i).zfill(3)
    files_names[key] = first_test_files[i-1]

  # Ball
  for i in range(1370, n+1370):
    key = "BA_" + str(i).zfill(3)
    files_names[key] = first_test_files[i-1]

  # Outer
  for i in range(5575, n+5575):
    key = "OR_" + str(i).zfill(3)
...
class IMS(database.Database):
  ...
  def download(self):
    ...
    if self.debug==0:
      ...
      files_names = files_debug(dirname)
